...
while True: 
    try: 
        ...
        cf = float(input("Qual o custo fixo/administrativo (%)? "))
        cv = float(input("Qual a comissão de vendas (%)? "))
        iv = float(input("Qual é o imposto sobre a venda (%)? "))
        ml = float(input("Qual a margem de lucro desejada (%)? "))
        
        #Formula de preço de Venda
